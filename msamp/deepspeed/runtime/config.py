...
class MSAMPDeepSpeedConfig(DeepSpeedConfig):
    ...
    def _do_error_check(self):
        ...
        assert self.msamp_optlevel in ['O1', 'O2', 'O3'],  \
            f'Invalid MS-AMP opt_level: {self.msamp_optlevel}, only O0, O1, O2, O3 are supported.' 
        if self.msamp_optlevel == 'O3':
            assert self.zero_enabled and self.zero_optimization_stage in [ZeroStageEnum.optimizer_states, ZeroStageEnum.gradients], \
                f'MS-AMP O3 requires ZeRO with optimizer_states or gradients partitioning.'
