...
class TeExtensionOverriderTestCase(unittest.TestCase):
    """Test TeExtension overrider."""
    def setUp(self):
    ...
    @decorator.cuda_test
    def test_fp8_cast_transpose_fused(self):
        """Test fp8_cast_transpose_fused."""
        # cast with torch.tensor
        input = torch.randn(self.size, device=self.device)
        scaling_meta = ScalingMeta(Dtypes.kfloat8_e4m3)
        scaling_meta.amax[0] = input.abs().max()
        ...
        cast_out1, transpose_out1 = texcpp.fp8_cast_transpose_fused(
            input, meta, tex.FP8FwdTensors.GEMM1_INPUT, fp8_type, None, None
        )
        assert torch.equal(cast_out1.t(), transpose_out1)
        ...
        cast_out2 = torch.randn(self.size, device=self.device)
        transpose_out2 = torch.randn(self.size, device=self.device)
        texcpp.fp8_cast_transpose_fused(
            scaling_input, meta, tex.FP8FwdTensors.GEMM1_INPUT, fp8_type, cast_out2, transpose_out2
        )
        assert torch.equal(cast_out2.t(), transpose_out2)

        assert torch.equal(cast_out1, cast_out2)
        assert torch.equal(transpose_out1, transpose_out2)
