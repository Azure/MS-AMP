...
class FP4QuantTestCase(unittest.TestCase):
    ...
    def test_fp4_quant(self):
        ...
        output_tensor = FP4_QUANT.quantize_simu_fp4_in_bf16(input_tensor, format='e2m1', nan_existed=False)
        ...
        output_tensor = FP4_QUANT.quantize_simu_fp4_in_bf16(input_tensor, format='e2m1', nan_existed=False, channel_wise=True)
        self.assertTrue(torch.allclose(output_tensor, target_tensor))
        
        output_tensor = FP4_QUANT.quantize_simu_fp4_in_bf16(input_tensor.view(-1, 3).T, format='e2m1', nan_existed=False, token_wise=True)      # token-wise outlier.
